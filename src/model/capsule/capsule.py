...
class RoutingLayer(nn.Module):
    ...
    def forward(self, x, random=True):
        ...
        priors = priors.transpose(1, 0).cuda()
        ...
        for i in range(num_iterations):
            probs = F.softmax(logits, dim=2).cuda()
...
class CapsuleNet(nn.Module):
    def __init__(self, args):
        ...
        self.routing_stats = RoutingLayer(num_input_capsules=3, num_output_capsules=2, data_in=8, data_out=4, num_iterations=2)
